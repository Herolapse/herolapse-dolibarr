 ...
 */
class pdf_herolapse_it extends ModelePDFFactures
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(["main", "bills", "products", "dict", "companies", "compta"]);
		...
		if (getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE") && $outputlangs->defaultlang != getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE")) {
			...
			$outputlangsbis->loadLangs(["main", "bills", "products", "dict", "companies", "compta"]);
		}
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if ($object->specimen) {
				...
				$client_name = preg_replace("/[\.]/", "", $object->thirdparty->name); // rimuovi i punti
				$client_name = preg_replace('/[\/\\\\:\*\?"<>\|]/', "", $client_name); // rimuovi altri caratteri vietati
				$client_name = str_replace(" ", "-", $client_name); // sostituisci gli spazi con trattini

				$filename = "Fattura-{$numero_fatt}_del_{$data_fattura}_{$client_name}.pdf";
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($object->lines[$i]->special_code == SUBTOTALS_SPECIAL_CODE) {
						...
						if ($sub_options) {
							if (isset($sub_options["titleshowuponpdf"])) {
								$pdf_sub_options["titleshowuponpdf"] =
									isset($pdf_sub_options["titleshowuponpdf"]) && $pdf_sub_options["titleshowuponpdf"] < $level ? $pdf_sub_options["titleshowuponpdf"] : $level;
					...
					if (
						$curY + 6 > $this->page_hauteur - $this->heightforfooter ||
						(isset($sub_options["titleforcepagebreak"]) && !($pdf->getNumPages() == 1 && $curY == $this->tab_top + $this->tabTitleHeight))
					) {
					...
					if ($this->getColumnStatus("photo")) {
						...
						$imageTopMargin = 1;
						if (isset($imglinesize["width"]) && isset($imglinesize["height"]) && $curY + $imageTopMargin + $imglinesize["height"] > $this->page_hauteur - $this->heightforfooter) {
					...
					if (getDolGlobalInt("PDF_INVOICE_SHOW_VAT_ANALYSIS")) {
						...
						$this->tva_array[$vatrate . ($vatcode ? " (" . $vatcode . ")" : "")] = [
							"vatrate" => $vatrate,
							"vatcode" => $vatcode,
							"amount" => $this->tva_array[$vatrate . ($vatcode ? " (" . $vatcode . ")" : "")]["amount"] + $tvaligne,
							"tot_ht" => $this->tva_array[$vatrate . ($vatcode ? " (" . $vatcode . ")" : "")]["tot_ht"] + $object->lines[$i]->total_ht,
						];
					} else {
					...
					if (file_exists($termsofsale) && is_readable($termsofsale)) {
						...
						for ($i = 1; $i <= $pagecount; $i++) {
							...
							if ($tplIdx !== false) {
								...
								setEventMessages(null, [$termsofsale . " cannot be added, probably protected PDF"], "warnings");
							}
						}
					}
				}
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (in_array($this->emetteur->country_code, ["FR"]) && empty($object->total_tva)) {
			$pdf->SetFont("", "", $default_font_size - 2);
			$pdf->SetXY($this->marge_gauche, $posy);
			if (empty($mysoc->tva_assuj)) {
				...
				getDolGlobalString("INVOICE_VAT_SHOW_REVERSE_CHARGE_MENTION") &&
				$this->emetteur->country_code != $object->thirdparty->country_code &&
				$this->emetteur->isInEEC() &&
		...
		if ($object->type != 2) {
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == "CB" || $object->mode_reglement_code == "VAD") {
				...
				if ($object->status != Facture::STATUS_DRAFT && $useonlinepayment) {
					...
					$langs->loadLangs(["payment", "paybox", "stripe"]);
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis)
	{
		...
		if (is_object($outputlangsbis)) {
			// When we show 2 languages we need more room for text, so we use a smaller font.
			$pdf->SetFont("", "", $default_font_size - 2);
		} else {
			$pdf->SetFont("", "", $default_font_size - 1);
		}
		...
		if (!empty($TPreviousIncoice)) {
			...
			foreach ($TPreviousIncoice as &$fac) {
				...
				$pdf->MultiCell(
					$col2x - $col1x,
					$tab2_hl,
					$outputlangs->transnoentities("PDFSituationTitle", (string) $fac->situation_counter) . " " . $outputlangs->transnoentities("TotalHT"),
					0,
					"L",
					true,
				);
			...
			$pdf->MultiCell(
				$col2x - $col1x,
				$tab2_hl,
				$outputlangs->transnoentities("PDFSituationTitle", (string) $object->situation_counter) . " " . $outputlangs->transnoentities("TotalHT"),
				0,
				"L",
				true,
			);
		...
		if ($total_line_remise > 0) {
			...
			$pdf->MultiCell(
				$col2x - $col1x,
				$tab2_hl,
				$outputlangs->transnoentities("TotalDiscount") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("TotalDiscount") : ""),
				0,
				"L",
				true,
			);
			...
			$pdf->MultiCell(
				$col2x - $col1x,
				$tab2_hl,
				$outputlangs->transnoentities("TotalHTBeforeDiscount") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("TotalHTBeforeDiscount") : ""),
				0,
				"L",
				true,
			);
		...
		if (!getDolGlobalString("MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT")) {
			...
			if (getDolGlobalString("MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL") && $tvaisnull) {
				...
				foreach ($this->localtax1 as $localtax_type => $localtax_rate) {
					if (in_array((string) $localtax_type, ["1", "3", "5"])) {
						continue;
					}
					...
					foreach ($localtax_rate as $tvakey => $tvaval) {
						if ($tvakey != 0) {
							...
							$totalvat =
								...
								(is_object($outputlangsbis) ? " / " . $outputlangsbis->transcountrynoentities("TotalLT1", $mysoc->country_code) : "");
							$totalvat .= " ";
							...
							$total_localtax =
								isModEnabled("multicurrency") && isset($object->multicurrency_tx) && $object->multicurrency_tx != 1 ? price2num($tvaval * $object->multicurrency_tx, "MT") : $tvaval;
					...
					foreach ($localtax_rate as $tvakey => $tvaval) {
						if ($tvakey != 0) {
							...
							$totalvat =
								...
								(is_object($outputlangsbis) ? " / " . $outputlangsbis->transcountrynoentities("TotalLT2", $mysoc->country_code) : "");
							$totalvat .= " ";
				...
				if (!getDolGlobalInt("PDF_INVOICE_SHOW_VAT_ANALYSIS")) {
					...
					foreach ($this->tva_array as $tvakey => $tvaval) {
						if ($tvakey != 0) {
							...
							$totalvat =
								$outputlangs->transcountrynoentities("TotalVAT", $mysoc->country_code) .
								(is_object($outputlangsbis) ? " / " . $outputlangsbis->transcountrynoentities("TotalVAT", $mysoc->country_code) : "");
							$totalvat .= " ";
				...
				foreach ($this->localtax1 as $localtax_type => $localtax_rate) {
					if (in_array((string) $localtax_type, ["2", "4", "6"])) {
						continue;
					}
				...
				if (price2num($object->revenuestamp, "MT") != 0) {
					...
					$pdf->MultiCell(
						$col2x - $col1x,
						$tab2_hl,
						$outputlangs->transnoentities("RevenueStamp") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("RevenueStamp", $mysoc->country_code) : ""),
						$useborder,
						"L",
						true,
					);
				...
				$pdf->MultiCell(
					$col2x - $col1x,
					$tab2_hl,
					$outputlangs->transnoentities("TotalTTC") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("TotalTTC") : ""),
					$useborder,
					"L",
					true,
				);
				...
				if (!isModEnabled("multicurrency") || $object->multicurrency_tx == 1 || getDolGlobalInt("MULTICURRENCY_SHOW_ALSO_MAIN_CURRENCY_ON_PDF") == 0) {
					...
					$pdf->MultiCell(
						$largcol2,
						$tab2_hl,
						"(" . price($sign * $total_ttc_origin, 0, $outputlangs, 1, -1, "MT", $mysoc->currency_code) . ")   " . price($sign * $total_ttc, 0, $outputlangs),
						$useborder,
						"R",
						true,
					);
				...
				if ($object->displayRetainedWarranty()) {
					...
					$retainedWarrantyToPayOn =
						$outputlangs->transnoentities("RetainedWarranty") .
						(is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("RetainedWarranty") : "") .
						" (" .
						$object->retained_warranty .
						"%)";
					$retainedWarrantyToPayOn .= !empty($object->retained_warranty_date_limit)
						? " " . $outputlangs->transnoentities("toPayOn", dol_print_date($object->retained_warranty_date_limit, "day"))
						: "";
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && !getDolGlobalString("INVOICE_NO_PAYMENT_DETAILS")) {
			...
			if (!isModEnabled("multicurrency") || $object->multicurrency_tx == 1 || getDolGlobalInt("MULTICURRENCY_SHOW_ALSO_MAIN_CURRENCY_ON_PDF") == 0) {
				...
				$pdf->MultiCell(
					$largcol2,
					$tab2_hl,
					"(" . price($deja_regle_origin + $depositsamount_origin, 0, $outputlangs, 1, -1, "MT", $mysoc->currency_code) . ")   " . price($deja_regle + $depositsamount, 0, $outputlangs),
					0,
					"R",
					false,
				);
			...
			if ($object->close_code == Facture::CLOSECODE_DISCOUNTVAT) {
				...
				$pdf->MultiCell(
					$col2x - $col1x,
					$tab2_hl,
					$outputlangs->transnoentities("EscompteOfferedShort") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("EscompteOfferedShort") : ""),
					$useborder,
					"L",
					true,
				);
			...
			$pdf->MultiCell(
				$col2x - $col1x,
				$tab2_hl,
				$outputlangs->transnoentities("RemainderToPay") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("RemainderToPay") : ""),
				$useborder,
				"L",
				true,
			);
			...
			if (!isModEnabled("multicurrency") || $object->multicurrency_tx == 1 || getDolGlobalInt("MULTICURRENCY_SHOW_ALSO_MAIN_CURRENCY_ON_PDF") == 0) {
				...
				$pdf->MultiCell(
					$largcol2,
					$tab2_hl,
					"(" . price($resteapayer_origin, 0, $outputlangs, 1, -1, "MT", $mysoc->currency_code) . ")   " . price($resteapayer, 0, $outputlangs),
					0,
					"R",
					true,
				);
		...
		$parameters = ["pdf" => &$pdf, "object" => &$object, "outputlangs" => $outputlangs, "index" => &$index, "posy" => $posy];
		...
		if (empty($hidetop)) {
			...
			if (getDolGlobalInt("INVOICE_CATEGORY_OF_OPERATION") == 1 && $this->categoryOfOperation >= 0) {
				$categoryOfOperations =
					$outputlangs->transnoentities("MentionCategoryOfOperations") . " : " . $outputlangs->transnoentities("MentionCategoryOfOperations" . $this->categoryOfOperation);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if ($showaddress) {
			...
			if (count($arrayidcontact) > 0) {
				...
				$labelbeforecontactname =
					$outputlangs->transnoentities("FromContactName") != "FromContactName" ? $outputlangs->transnoentities("FromContactName") : $outputlangs->transnoentities("Name");
			...
			if (
				$usecontact &&
				($object->contact->socid != $object->thirdparty->id && (!isset($conf->global->MAIN_USE_COMPANY_NAME_OF_CONTACT) || getDolGlobalString("MAIN_USE_COMPANY_NAME_OF_CONTACT")))
			) {
		...
		$pagehead = ["top_shift" => $top_shift, "shipp_shift" => $shipp_shift];
