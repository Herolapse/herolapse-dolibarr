 ...
 */
class pdf_herolapse_it extends ModelePDFPropales
{
	/**
	...
	public function write_file($object, $outputlangs, $srctemplatepath = "", $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$langfiles = ["main", "dict", "companies", "bills", "products", "propal", "compta"];
		...
		if ($conf->propal->multidir_output[$conf->entity]) {
			...
			if ($object->specimen) {
				...
				$filename = "Preventivo-{$numero_prev}_del_{$data_preventivo}_{$client_name}.pdf";
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if (
						$curY + 6 > $this->page_hauteur - $heightforfooter ||
	...
	public function drawInfoTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (!empty($object->delivery_date)) {
			...
			$pdf->SetXY($posxval, $posy);
			$lib_availability =
				$outputlangs->transnoentities("AvailabilityType" . $object->availability_code) != "AvailabilityType" . $object->availability_code
					? $outputlangs->transnoentities("AvailabilityType" . $object->availability_code)
					: $outputlangs->convToOutputCharset($object->availability);
		...
		if (!getDolGlobalString("PROPOSAL_PDF_HIDE_DELIVERYMODE") && $object->shipping_method_id > 0) {
			...
			$lib_condition_paiement =
				$outputlangs->transnoentities("SendingMethod" . strtoupper($shipping_method_code)) != "SendingMethod" . strtoupper($shipping_method_code)
					? $outputlangs->trans("SendingMethod" . strtoupper($shipping_method_code))
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		if (getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE") && $outputlangs->defaultlang != getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE")) {
			$outputlangsbis = new Translate("", $conf);
			$outputlangsbis->setDefaultLang(getDolGlobalString("PDF_USE_ALSO_LANGUAGE_CODE"));
			$outputlangsbis->loadLangs(["main", "dict", "companies", "bills", "products", "propal"]);
			$default_font_size--;
		}
		...
		*/
		...
		if ($deja_regle > 0) {
			...
			$pdf->MultiCell(
				$col2x - $col1x,
				$tab2_hl,
				$outputlangs->transnoentities("AlreadyPaid") . (is_object($outputlangsbis) ? " / " . $outputlangsbis->transnoentities("AlreadyPaid") : ""),
				0,
				"L",
				false,
			);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		$outputlangs->loadLangs(["main", "propal", "companies", "bills"]);
